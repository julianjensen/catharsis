...
Describer.prototype.elements = function(type, useLongFormat, keyName) {
	var context = new Context();
	var items = type.elements.slice(0);
	var result = new Result();

	addModifiers(this, context, result, type, useLongFormat);
	result.description = this._combineMultiple(items, context, keyName, 'element', useLongFormat);
...
Describer.prototype.type = function(type, useLongFormat) {
	...
	switch (type.type) {
		...
		case Types.TypeUnion:
			result = this.elements(type, useLongFormat, 'union');
			break;
        case Types.TypeIntersection:
            result = this.elements(type, useLongFormat, 'intersection');
            break;
        case Types.TypeTuple:
            result = this.elements(type, useLongFormat, 'tuple');
            break;
